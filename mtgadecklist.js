...
var cardgroups = {
  "creature": ["Creature"],
  "instant": ["Instant", "Sorcery", "Enchantment", "Planeswalker", "Artifact"],
  "land": ["Land"]
}
var groupnames = {
...
function showDecklist(deck, node) {
  ...
  var cardtemplate = "<div class=\"card\" onmouseover=\"javascript:showCardPreview(this, '{0}')\" style=\"background-image:url('{1}');box-shadow:inset 0 0 5px 2px {2};\" /><span class=\"quantity q-{3}\">{4}</span><span class=\"name\">{5}</span><span class=\"mc\">{6}</span></div>";
  var cardcnt, card, art, colors, mc, partialhtml;
  var rarities = {"common":0,"uncommon":0,"rare":0,"mythic":0};
  for (var g in cardgroups) {
    partialhtml = "";
    cardcnt = 0;
    for (var c = 0; c < deck.main.length; c++) {
      card = deck.main[c];
      type = card.data.card_faces ? card.data.card_faces[0].type_line : card.data.type_line;
      if (typeinGroup(type, cardgroups[g])) {
        ...
        partialhtml += cardtemplate.format(art, art, getBorderColor(colors), card.data.rarity, card.quantity, card.name, mcsymbols);
      }
    }
    if (cardcnt > 0) {
      html += grouptemplate.format(cardcnt, (groupnames[g][cardcnt] || groupnames[g][0]), g) + partialhtml;
    }
  }
