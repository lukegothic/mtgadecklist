// Utils
String.prototype.format = String.prototype.format || function() {
  var s = this;
  for (var i = 0; i < arguments.length; i++) {
    var reg = new RegExp("\\{" + i + "\\}", "gm");
    s = s.replace(reg, arguments[i]);
  }
  return s;
};
String.prototype.padStart = String.prototype.padStart || function(targetLength,padString) {
    targetLength = targetLength>>0; //truncate if number or convert non-number to 0;
    padString = String((typeof padString !== 'undefined' ? padString : ' '));
    if (this.length > targetLength) {
        return String(this);
    }
    else {
        targetLength = targetLength-this.length;
        if (targetLength > padString.length) {
            padString += padString.repeat(targetLength/padString.length); //append to original to ensure we are longer than needed
        }
        return padString.slice(0,targetLength) + String(this);
    }
};
// settings
var defaultSettings = {
  header: "1",
  footer: "1",
  theme: "dark",
  hscards: "0",
  layout: "single",
  groups: "normal",
  preview: "inside"
};
function getDecklistSettings(node) {
  var settings = {};
  if (node.dataset.settings) {
    var aSettings = node.dataset.settings.split(";");
    var settings = {};
    var setting;
    for (var s = 0; s < aSettings.length; s++) {
      setting = aSettings[s].split("=");
      settings[setting[0]] = setting[1];
    }
  }
  for (s in defaultSettings) {
    settings[s] = settings[s] || defaultSettings[s];
  }
  return settings;
}
// setting.groups
var GroupInfo = {
  "creature": {
    "name": ["Creatures", "Creature"],
    "icon": "creature"
  },
  "planeswalker": {
    "name": ["Planeswalkers", "Planeswalker"],
    "icon": "planeswalker"
  },
  "instant": {
    "name": ["Instants", "Instant"],
    "icon": "instant"
  },
  "sorcery": {
    "name": ["Sorceries", "Sorcery"],
    "icon": "sorcery"
  },
  "artifact": {
    "name": ["Creatures", "Creature"],
    "icon": "creature"
  },
  "enchantment": {
    "name": ["Enchantments", "Enchantment"],
    "icon": "enchantment"
  },
  "land": {
    "name": ["Lands", "Land"],
    "icon": "land"
  },
  "spell": {
    "name": ["Spells", "Spell"],
    "icon": "instant"
  },
  "mixed": {
    "name": ["Cards", "Card"],
    "icon": "chaos"
  }
}
var cardGroups = {
  "bytype": {
    "creature": ["Creature"],
    "planeswalker": ["Planeswalker"],
    "instant": ["Instant"],
    "sorcery": ["Sorcery"],
    "artifact": ["Artifact"],
    "enchantment": ["Enchantment"],
    "land": ["Land"]
  },
  "normal": {
    "creature": ["Creature"],
    "spell": ["Instant", "Sorcery", "Enchantment", "Planeswalker", "Artifact"],
    "land": ["Land"]
  },
  "ungrouped": {
    "mixed": ["Creature", "Instant", "Sorcery", "Enchantment", "Planeswalker", "Artifact", "Land"]
  }
}
var groupnames = {
  "creature": ["Creatures", "Creature"],
  "instant": ["Spells", "Spell"],
  "land": ["Lands", "Land"]
}
function typeinGroup(cardtype, grouptype) {
  for (var t = 0; t < grouptype.length; t++) {
    if (cardtype.indexOf(grouptype[t]) !== -1) {
      return true;
    }
  }
  return false;
}
// sorting and htmlcolor data for each MTG color
var ColorInfo = {
    "W": { order: 1, hex: "#fdbd35" },
    "U": { order: 2, hex: "#018dc8" },
    "B": { order: 3, hex: "#3d3a33" },
    "R": { order: 4, hex: "#b7032a" },
    "G": { order: 5, hex: "#2c5535" },
    "M": { order: 6, hex: "#c2c20a" },
    "A": { order: 7, hex: "#777777" },
    "L": { order: 8, hex: "#777777" }
};
function getColorInfo(card) {
  card = (card.card_faces && card.card_faces[0]) || card;
  switch(card.colors.length) {
    case 0: return (card.type_line.indexOf("Land") !== -1 ? ColorInfo.L : ColorInfo.A);
    case 1: return ColorInfo[card.colors[0]];
    default: return ColorInfo.M;
  }
}
// mixed sorting expression for cards, cmc + color + name
function getSortExpression(card) {
  return "{0}_{1}_{2}".format(("" + card.cmc).padStart(2, "0"), getColorInfo(card).order, card.name.toLowerCase());
}
function showCardPreview(card, url) {
  var cardpreview = document.getElementById("cardpreview");
  if (cardpreview === null) {
    cardpreview = document.createElement("img");
    cardpreview.id = "cardpreview";
    cardpreview.width = "250";
    cardpreview.addEventListener("mouseout", hideCardPreview);
    cardpreview.addEventListener("mousemove", function(e) {
      if (e.y > parseInt(cardpreview.dataset.hideoffset) + 28) {
        hideCardPreview();
      }
    });
    document.body.appendChild(cardpreview);
  }
  var bcr = card.getBoundingClientRect();
  cardpreview.style.top = bcr.top + document.body.scrollTop;
  cardpreview.style.left = bcr.left + document.body.scrollLeft;
  cardpreview.dataset.hideoffset = bcr.top;
  cardpreview.src = url;
  cardpreview.style.display = "block";
}
function hideCardPreview() {
  var cardpreview = document.getElementById("cardpreview");
  if (cardpreview !== null) {
    cardpreview.style.display = "none";
  }
}
function parseDecklist(node) {
  var reCard = /x?(\d+)x?.?(.*?).?\((.*?)\).?(\d+)/g;
  var decklist = {
    main: [],
    side: []
  };
  do {
    card = reCard.exec(node.innerHTML);
    if (card) {
      if (card !== null) {
        decklist.main.push({
          "quantity": parseInt(card[1]),
          "name": card[2].trim(),
          "set": card[3].toLowerCase(),
          "collector_number": parseInt(card[4])
        });
      }
    }
  } while (card);
  var scryfallQ = [];
  for (var c = 0; c < decklist.main.length; c++) {
    card = decklist.main[c];
    scryfallQ.push("(s:" + card.set + " and cn:" + card.collector_number + ")");
  }
  // scryfall esta limitado a 20 nested queries, dividimos peticion en chunks de 20
  var scryfallQlimit = 20;
  var oReqs = [];
  while (scryfallQ.length > 0) {
    var oReq = new XMLHttpRequest();
    oReq.addEventListener("load", function() {
      var scryfallData = JSON.parse(this.responseText).data;
      var data;
      for (var c = 0; c < decklist.main.length; c++) {
        card = decklist.main[c];
        for (var di = 0; di < scryfallData.length; di++) {
          data = scryfallData[di];
          if (card.set === data.set && card.collector_number == data.collector_number) {
            card.data = data;
            break;
          }
        }
      }
      var allReqFinished = true;
      for (var r = 0; r < oReqs.length; r++) {
        if (oReqs[r].readyState !== oReqs[r].DONE) {
          allReqFinished = false;
          break;
        }
      }
      if (allReqFinished) {
        showDecklist(decklist, node);
      }
    });
    oReq.open("GET", encodeURI("https://api.scryfall.com/cards/search?q=" + scryfallQ.splice(0, Math.min(scryfallQ.length, scryfallQlimit)).join("or")));
    oReqs.push(oReq);
    oReq.send();
  }
}
function showDecklist(deck, node) {
  for (var part in deck) {
    deck[part].sort(function(a, b) {
      var sa = getSortExpression(a.data);
      var sb = getSortExpression(b.data);
      return sa < sb ? -1 : 1;
    });
  }
  var settings = getDecklistSettings(node);
  var groups = cardGroups[settings.groups];
  var html = "";
  var grouptemplate = "<div class=\"card card-group\"><span class=\"quantity\">{0}</span><span class=\"name\">{1}</span><i class=\"mc ms ms-{2}\"></i></div>"
  var cardtemplate = "<div class=\"card\" onmouseover=\"javascript:showCardPreview(this, '{0}')\" style=\"background-image:url('{1}');box-shadow:inset 0 0 0 2px {2};\" /><span class=\"quantity q-{3}\">{4}</span><span class=\"name\">{5}</span><span class=\"mc\">{6}</span></div>";
  var cardcnt, card, art, mc, grouphtml;
  var summary = {
    "rarities": { "common":0, "uncommon":0, "rare":0, "mythic":0 },
  };
  for (var g in groups) {
    grouphtml = "";
    cardcnt = 0;
    for (var c = 0; c < deck.main.length; c++) {
      card = deck.main[c];
      data = (card.data.card_faces && card.data.card_faces[0]) || card.data;
      if (typeinGroup(data.type_line, groups[g])) {
        cardcnt += card.quantity;
        summary.rarities[card.data.rarity] += card.quantity;
        art = data.image_uris.normal;
        mc = data.mana_cost;
        mcsymbols = mc.toLowerCase().replace(/\{/g, "<i class=\"ms ms-cost ms-shadow ms-").replace(/\}/g, "\"></i>");
        grouphtml += cardtemplate.format(art, art, getColorInfo(card.data).hex, card.data.rarity, card.quantity, card.name, mcsymbols);
      }
    }
    if (cardcnt > 0) {
      html += grouptemplate.format(cardcnt, (GroupInfo[g].name[cardcnt] || GroupInfo[g].name[0]), GroupInfo[g].icon) + grouphtml;
    }
  }
  var deckfooter = "<div class=\"footer\">";
  for (var r in summary.rarities) {
    if (summary.rarities[r] > 0) {
      deckfooter += "<span>" + summary.rarities[r] + "</span><span class=\"wc\ wc-" + r + "\"></span>";
    }
  }
  deckfooter += "</div>"
  node.className = "decklist";
  node.innerHTML = html + deckfooter;
}

var decklists = document.querySelectorAll("[data-decklist]");
if (decklists.length > 0) {
  var link = document.createElement('link');
  link.href = "//cdn.jsdelivr.net/npm/mana-font@latest/css/mana.min.css";
  link.rel ="stylesheet";
  document.head.appendChild(link);
  link = document.createElement('link');
  link.href = "//fonts.googleapis.com/css?family=Montserrat";
  link.rel ="stylesheet";
  document.head.appendChild(link);
  // inject css
  var style = document.createElement('style');
  style.innerHTML = ".card.card-group {color:#fff; background-color: #1c1c1c;}#cardpreview{position:absolute;z-index:999999;border-radius:10px;}.decklist{width:250px;}.card{box-sizing:border-box;position:relative;height:30px;width:100%;overflow:hidden;background-position:50% 25%;border:3px solid #1c1c1c;border-top-left-radius:10px;border-top-right-radius:10px;margin-top:-8px;}.card .quantity{font-size:0.8em;font-family:'Montserrat',sans-serif;position:absolute;top:0;left:0;width:18px;font-weight:bold;height:20px;line-height:20px;text-align:right;text-shadow: 0px 0px 3px #000000, 0px 0px 2px #000000, 0px 0px 1px #000000;}.card .quantity.q-common{color:#eeedee}.card .quantity.q-uncommon{color:#a4c0cf}.card .quantity.q-rare{color:#f0bd38}.card .quantity.q-mythic{color:#f95e34}.card .name{font-size:0.8em;font-family:'Montserrat',sans-serif;position:absolute;top:0;left:22px;color:#ffffff;font-weight:bold;height:20px;line-height:20px;text-shadow: 0px 0px 3px #000000, 0px 0px 2px #000000, 0px 0px 1px #000000;}.card .mc{position:absolute;top:0;right:4px;height:20px;line-height:20px;}.card span.mc{top:4px;}.decklist .footer{position:relative;text-align:center;background-color:#1c1c1c;margin-top:-8px;padding:1px;border-bottom-left-radius:10px;border-bottom-right-radius:10px;}.decklist .footer span{font-size:0.8em;font-family:'Montserrat',sans-serif;font-weight:bold;color:#ffffff;}.decklist .footer .wc{margin:0 2px;font-weight:bold;height:20px;width:20px;display:inline-block;vertical-align:text-bottom;background-size:cover;}.wc.wc-common{background-image:url('data:image/png;base64,')}.wc.wc-uncommon{background-image:url('data:image/png;base64,')}.wc.wc-rare{background-image:url('data:image/png;base64,')}.wc.wc-mythic{background-image:url('data:image/png;base64,')}.ms-cost{font-size:.65em;}";
  document.head.appendChild(style);
  // process decklists
  for (var d = 0; d < decklists.length; d++) {
    parseDecklist(decklists[d]);
  }
}
