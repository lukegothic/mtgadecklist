...
function showDecklist(deck, node) {
  ...
  // TODO: settings
  var html = "";
  var template = "<div class=\"card\" onmouseover=\"javascript:showCardPreview(this, '{0}')\" style=\"background-image:url('{1}');box-shadow:inset 0 0 5px 2px {2};\" /><span class=\"quantity\">{3}</span><span class=\"name\">{4}</span><span class=\"mc\">{5}</span></div>";
  ...
  for (var c = 0; c < deck.main.length; c++) {
    ...
    html += template.format(art, art, getBorderColor(colors), card.quantity, card.name, mcsymbols);
  }
