...
function getBorderColor(colors) {
  if (colors.length > 1) {
    return mtgcolorToHEX["M"];
  } else {
    return mtgcolorToHEX[colors[0]];
  }
...
function showDecklist(deck, node) {
  ...
  // TODO: settings
  var html = "";
  var template = "<div class=\"card\" style=\"background-image:url({0});box-shadow:inset 0 0 0 2px {1};\" /><span class=\"quantity\">{2}</span><span class=\"name\">{3}</span><span class=\"mc\">{4}</span></div>";
  ...
  for (var c = 0; c < deck.main.length; c++) {
    ...
    art = card.data.card_faces ? card.data.card_faces[0].image_uris.art_crop : card.data.image_uris.art_crop;
    ...
    html += template.format(art, getBorderColor(colors), card.quantity, card.name, mcsymbols);
  }
